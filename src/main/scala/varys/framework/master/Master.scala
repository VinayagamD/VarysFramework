...
import varys.framework.master.scheduler.{CoflowScheduler, SchedulerInput}
import varys.framework.master.ui.MasterWebUI
import varys.framework.{AddFlow, AddFlows, BestRxMachines, BestTxMachines, CheckForSlaveTimeOut, CoflowDescription, CoflowState, DeleteFlow, FlowProgress, GetFlow, GetFlows, GotFlowDesc, GotFlowDescs, Heartbeat, MasterState, RegisterClient, RegisterClientFailed, RegisterCoflow, RegisterCoflowFailed, RegisterSlave, RegisterSlaveFailed, RegisteredClient, RegisteredCoflow, RegisteredSlave, RejectedCoflow, RequestBestRxMachines, RequestBestTxMachines, RequestMasterState, RequestWebUIPort, ScheduleRequest, UnregisterCoflow, UpdatedRates, WebUIPortResponse}
  ...
  def start(): (ActorSystem, Int) = {
    ...
    val actor = actorSystem.actorOf(
      Props(new MasterActor(host, boundPort, webUiPort)).withRouter(
        RoundRobinPool(nrOfInstances = NUM_MASTER_INSTANCES)),
      name = actorName)
