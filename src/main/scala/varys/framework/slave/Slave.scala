...
import varys.framework.{AddFlow, AddFlows, DataServer, DataType, DeleteFlow, FlowDescription, GetFlow, GetFlows, Heartbeat, RegisterSlave, RegisterSlaveFailed, RegisteredCoflow, RegisteredSlave, RequestSlaveState, SlaveState, UnregisterCoflow}
...
private[varys] class SlaveActor(
  ...
  implicit var futureExecContext: ExecutionContextExecutor = ExecutionContext.fromExecutor(Utils.newDaemonCachedThreadPool())
